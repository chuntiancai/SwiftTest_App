...
/**
    1.字节和地址
        内存以“字节为单位”，里面的每个小框框就代表着内存中的一个字节，白色数字就是每个字节的地址。可以发现，内存中相邻字节的地址是连续的。
    ...
    2.变量的存储
        ...
        变量存储单元的第一个字节的地址就是该变量的地址，任何变量在内存中都是以二进制的形式存储。
        一个负数的二进制形式，其实就是对它的正数的二进制形式进行取反后再+1。（取反的意思就是0变1、1变0）
    ...
    3. 变量在内存中存储的原则
        ...
        方法栈里的变量是先分配字节地址大内存,然后分配字节地址小的内存(内存寻址是由大到小)：
            因为内存寻址是由大到小，所以先定义的变量的内存地址会比后定义的大。
            number1地址 > number2
    
        变量的首地址,是变量所占存储空间字节地址最小的那个地址。
 
        低位保存在低地址字节上,高位保存在高地址字节上：(注意，是字节)
