...
//MARK: 字符串拼接，需要#include <string.h>
void testStrcat(void){
    
    // 1.字符串拼接原理 : 首先遍历第一个字符串,直到遇到\0为止, 然后取出第二个字符串中的字符, 从\0的位置开始添加, 添加完毕之后再在最后添加一个\0
...
//                  str1能够存放的元素个数            -   str1已经存放的个数  - \0
    size_t length = sizeof(str1) / sizeof(str1[0]) - strlen(str1) - 1;  //size_t 无符号整数，机器字长。
    printf("length = %lu\n", length);
