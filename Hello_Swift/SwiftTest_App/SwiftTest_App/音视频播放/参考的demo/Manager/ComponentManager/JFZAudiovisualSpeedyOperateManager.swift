//
//  JFZAudiovisualSpeedyOperateManager.swift
//  SwiftNote_App
//
//  Created by mathew on 2021/7/13.
//  Copyright © 2021 com.mathew. All rights reserved.
//
//  播放器快捷操作的事件管理器，也是弱引用播放页面对象

import Foundation
import MediaPlayer

class JFZAudiovisualSpeedyOperateManager {
    ...
    enum SpeedyType {
        ...
        var remindImg: UIImage? {
            switch self {
            case .fastForward:
                return UIImage(named: "hy_video_fastForward", in: JFZ_PLAYER_SOURCE_BUNDLE, compatibleWith: nil)
            case .fastRewind:
                return UIImage(named: "hy_video_fastRewind", in: JFZ_PLAYER_SOURCE_BUNDLE, compatibleWith: nil)
            case .lightingUp, .lightingDown:
                return UIImage(named: "hy_video_light", in: JFZ_PLAYER_SOURCE_BUNDLE, compatibleWith: nil)
            default:
                return nil
            }
        }
    ...
    weak var playerView: JFZPlayerCommonView?
...
//        v.showsRouteButton = true
    ...
    func movedDrag(point: CGPoint) {
        ...
        else if startPoint.x < JFZ_SCREEN_WIDTH / 2
