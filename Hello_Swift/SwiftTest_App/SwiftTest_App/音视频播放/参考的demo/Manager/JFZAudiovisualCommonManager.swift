//
//  JFZAudiovisualCommonManager.swift
//  SwiftNote_App
//
//  Created by mathew on 2021/7/13.
//  Copyright © 2021 com.mathew. All rights reserved.
//
// 播放的普通事件的管理器，需要弱引用 播放页面JFZPlayerCommonView对象
...
class JFZAudiovisualCommonManager: NSObject {
    
    weak var playerView: JFZPlayerCommonView?    //被管理的播放页面
    
    /// 隐藏状态栏计时器
    var hideTimer: Timer?
    
    /// 当前音视频配置
    var playerConfig: JFZPlayerCommonConfig? {
        willSet {
            if let newConfig = newValue {// 判断是否为第一次初始化，否则判断是否进行断点续播存储，这里不是第一次初始化
                ...
                if let oldConfig = playerConfig {
                        ...
                        (oldConfig.audioUrl == newConfig.audioUrl && oldConfig.videoUrl == newConfig.videoUrl)
                    {
                        ...
                        setPlayContinue()   //根据url存储断点播放时间
                    }
                } else {
                    
                    if JFZNetReach.isReachableViaWWAN() {
                        print("当前使用手机流量")
    ...
    /// 当前播放器状态
    var playerStatus: JFZAudiovisualStatus? {
        didSet {
    ...
    /// 断点续播节点列表
    private var playContiuneListPath: String = JFZPlayerFileDirectory.PlayContinue.listPlist
    /// 当前播放地址
    ...
    convenience init(_ view: JFZPlayerCommonView) {
        self.init()
    ...
    /** 初始化播放器*/
    func updatePlayerItem(config: JFZPlayerCommonConfig) {
        // 设置全屏标题
        ...
        // 初始化播放器
        if let urlStr = config.videoUrl, urlStr != "" { //视频
            playVideo(urlStr: urlStr, config: config)
        } else if let urlStr = config.audioUrl, urlStr != "" {  //音频
            playVideo(urlStr: urlStr, config: config)
        }
    }
    ...
    /** 播放视频*/
    private func playVideo(urlStr: String, config: JFZPlayerCommonConfig) {
...
//                playerView?.playerLayer?.videoGravity = .resizeAspectFill
            ...
            playerView?.noNetView.isHidden = JFZNetReach.isReachable()
...
//                playerStatus = .pause
                playerStatus = .playing
//                playerView?.noNetView.isHidden = false
                playerView?.noNetView.isHidden = true
            } else {
                /// 进行视频的播放
                if JFZNetReach.isReachableViaWWAN() {
...
//                    playerStatus = .pause
            ...
            if config.playContinue {
                if let playContinueList = NSDictionary(contentsOfFile: playContiuneListPath) {
                    ...
                    if let playContinueTime = playContinueList[urlStr.JFZmd5] as? Double,
        ...
        if config.needCache && !JFZNetReach.isReachableViaWWAN() && isOnlineSource {
            // 缓存的处理
            let location = JFZDefaultVideoCacheLocation(remoteURL: playerUrl, mediaType: isVideo ? .video : .audio, authenticationFunc: config.authenticationFunc)
            let canCache = JFZPlayerCacheStorage.available > Float(500 * 1024 * 1024)
                ...
                if let asset = cache.asset {
                    ...
                    DispatchQueue.main.async {
                        ...
                        if self.playerView?.videoCacher.cacheList.contains(cache.identifier) == true {
                            self.playerView?.controlPanel.cacheView.frame = CGRect(x: 48, y: 20, width: JFZ_SCREEN_WIDTH - 176, height: 1)
                        }
                    }
        ...
        if let size = videoSize {
            ...
            if isVertical {
                ...
                if isFullScreen {
                    ...
                    //FIXME: 暂时修改屏幕的尺寸
//                    return CGRect(x: 0, y: 0, width: UIScreen.main.bounds.size.width, height: playerHeight)
                }
            }
        }
        
        
        return CGRect(x: 0, y: 0, width: JFZ_SCREEN_WIDTH, height: JFZ_SCREEN_WIDTH / 16 * 9)
    }
    ...
    func setPlayContinue() {
            ...
            let currentTime = playerView?.videoPlayer?.currentItem?.currentTime().seconds
        {
            if let playContinueList = NSDictionary(contentsOfFile: playContiuneListPath),
                ...
                // 已存在记录 -> 添加
                playContinueDic[urlStr.JFZmd5] = currentTime
                (playContinueDic as NSDictionary).write(toFile: playContiuneListPath, atomically: true)
            } else {
                // 未存在 —> 新增
                ([urlStr.JFZmd5: currentTime] as NSDictionary).write(toFile: playContiuneListPath, atomically: true)
            }
        }
    }
}
...
//MARK: - 控制面板相关
extension JFZAudiovisualCommonManager {
    /** 刷新播放器控制面板（进度｜时间）*/
