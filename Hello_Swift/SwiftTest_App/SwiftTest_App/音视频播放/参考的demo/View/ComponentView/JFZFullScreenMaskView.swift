...
protocol JFZFullScreenMaskViewDelegate: NSObjectProtocol {
...
class JFZFullScreenMaskView: UIView {
    
    weak var delegate: JFZFullScreenMaskViewDelegate?
    
    /// 全屏锁定
    var isScreenLock = false {
        didSet {
            lockBtn.setImage(UIImage(named: isScreenLock ? "hy_fullscreen_lock" : "hy_fullscreen_unlock", in: JFZ_PLAYER_SOURCE_BUNDLE, compatibleWith: nil), for: .normal)
        }
    }
    ...
    /// 退出全屏
    var backBtn: JFZResponseExpandButtton = {
        let btn = JFZResponseExpandButtton.init(type: .custom)
        btn.setImage(UIImage(named: "hy_video_ic_back", in: JFZ_PLAYER_SOURCE_BUNDLE, compatibleWith: nil), for: .normal)
        return btn
    }()
    ...
    /// 更多功能
    var moreBtn: JFZResponseExpandButtton = {
        let btn = JFZResponseExpandButtton()
        btn.setImage(UIImage(named: "hy_fullscreen_ic_more", in: JFZ_PLAYER_SOURCE_BUNDLE, compatibleWith: nil), for: .normal)
        return btn
    }()
    
    /// 屏幕锁定按钮
    var lockBtn: JFZResponseExpandButtton = {
        let btn = JFZResponseExpandButtton()
        btn.setImage(UIImage(named: "hy_fullscreen_unlock", in: JFZ_PLAYER_SOURCE_BUNDLE, compatibleWith: nil), for: .normal)
        return btn
    }()
    ...
    private var speedCollectionView: UICollectionView = { () -> UICollectionView in
        ...
        collectionView.register(JFZMoreFunctionSpeedCollectionViewCell.self, forCellWithReuseIdentifier: "JFZMoreFunctionSpeedCollectionViewCell")
    ...
    private func createMoreFunctionView() {
        moreFunctionView.frame = CGRect(x: JFZ_SCREEN_HEIGHT, y: 64, width: 100, height: JFZ_SCREEN_WIDTH - (JFZ_IS_IPHONEX ? 119 : 104))
    ...
    func showMoreFunctionView(isVerticalScreen: Bool) {
        ...
        UIView.animate(withDuration: 0.2) {
            if isVerticalScreen {
                self.moreFunctionView.frame = CGRect(x: JFZ_SCREEN_WIDTH - self.moreFunctionWidth, y: 64, width: self.moreFunctionWidth, height: JFZ_SCREEN_HEIGHT - (JFZ_IS_IPHONEX ? 119 : 104))
            } else {
                self.moreFunctionView.frame = CGRect(x: JFZ_SCREEN_HEIGHT - self.moreFunctionWidth, y: 64, width: self.moreFunctionWidth, height: JFZ_SCREEN_WIDTH - (JFZ_IS_IPHONEX ? 119 : 104))
            }
        }
    }
    ...
    func hidMoreFunctionView() {
        ...
        UIView.animate(withDuration: 0.2) {
            self.moreFunctionView.frame = CGRect(x: JFZ_SCREEN_HEIGHT, y: 64, width: self.moreFunctionWidth, height: JFZ_SCREEN_WIDTH - (JFZ_IS_IPHONEX ? 119 : 104))
        }
    }
...
extension JFZFullScreenMaskView: UICollectionViewDelegate, UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return speedArray.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "JFZMoreFunctionSpeedCollectionViewCell", for: indexPath) as! JFZMoreFunctionSpeedCollectionViewCell
