...
//MARK: - 笔记
/**
    1、GCD是一个任务调度框架，是基于线程池的模式执行并发任务的模式，主要是以队列的模式来管理并执行任务。
        你无法获知GCD会把任务分配到哪一个线程中执行，但是你可以指定任务在哪个队列执行，这就是GCD的队列模式。
 
    2、GCD可以通过DispatchQueue来创建串行、并行队列。
        在GCD中，任务被封装在async的block中，你也可以通过workItem来创建任务。
        你可以在创建workItem的时候传入.barrier栅栏参数，用于控制该任务前面的任务全部执行完之后，才可以执行队列后的任务。
        线程安全：通过DispatchSemaphore创建信号量，用于控制任务争夺资源的问题，在临界区代码PV操作，从而实现线程安全，信号量是资源的数目。
        GCD可以把不同队列的任务放到统一组DispatchGroup中，组内的任务执行完之后，再发通知，实现任务捆绑功能。
        GCD的主队列是与主线程绑定的队列，也就是主队列中的所有任务都会分配给主线程执行。
        GCD的全局并发队列，则是系统提供给你使用的，会在多个线程中并发执行。
        GCD与主线程的通信，就是在你的任务闭包中添加主队列任务，例如执行主队列的闭包。
        GCD还可以设置不同队列的优先级，挂起唤醒其他队列等功能，例如suspend，resume等函数。
...
/**
    ...
    4、同步和异步：
        ...
        async: 不阻塞当前线程，就是当前线程跳过我async名下的任务，只需要开启一下我async名下的任务就可以了，你就继续走你的代码，
                我async被开启后，怎么来执行，那是async所需要关心的了，反正不关你当前线程的事。
    ...
    7、GCD中的队列任务组：
        所谓的队列任务组，就是多个队列的任务为一组，也可以多个队列的多个任务为一组，就不一定整个队列都在组里。等一组的队列执行完之后，组对象会发出通知，这时你可以指定通知谁。所以就是：
                    1、是队列自己把一些任务加入到组里的，不是组来要队列的任务的。
                    2、是组对象的方法来监听加入组内的队列任务有没有执行完，然后去通知指定队列的。
