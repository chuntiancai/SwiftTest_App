...
//MARK: - 笔记
/**
    1、NSthread是苹果提供的操作线程的api，需要程序员自己去管理线程的生命周期。
        需要自己手动去调用thread的start方法来执行任务，你可以子类化Thread，在main方法里添加自己的任务，也可以在Thread的init方法中写自己的任务。
        thread有strat、sleep、exit等方法控制线程的生命周期，执行完任务后，thread会自动结束自己的生命。
        线程安全：通过pthread_mutex互斥锁，objc同步锁等在临界区代码加锁，从而实现线程安全。
        线程通信：通过NSobject的performSelector方法可以指定任务在指定线程中执行，从而实现通信。
        Thread中有提供一些接口，例如是否是主线程、线程是否相等、获取主线程等，用于判断当前线程。
 */
/**
    1、主线程的作用：
        显示\刷新UI界面。
    ...
    3、self.perform(, with: , afterDelay: ) 方法底层是添加到runloop里执行的，需要注意的是， 子线程默认是没有开启runloop的， 所以添加了也没用。
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        case 2:
            ...
            /**
                ...
                2、自旋锁是忙等的，然后线程也给了优先级，高优先级的线程可以占用cpu不让其他线程运行，低优先级的线程占用锁，所以导致了死锁问题。也就是优先级反转问题。
             
                3、pthread_mutex 互斥锁，等待锁的线程会处于休眠状态。这是一个C语言的结构体。
             
                4、递归锁，用于递归函数中的锁，修改pthread_mutex的属性即可变成递归锁。只允许同一个线程对递归锁多次加锁，不同线程则需要等待其他线程跑完才能获取锁。
             
                5、@synchronized 是对mutex递归锁的封装，不推荐使用，性能差。
                
                6、C语言还提供了读写锁pthread_rwlock。dispatch_barrier_async：异步栅栏调用，但必须是自己创建的并发队列，不能是全局队列。
