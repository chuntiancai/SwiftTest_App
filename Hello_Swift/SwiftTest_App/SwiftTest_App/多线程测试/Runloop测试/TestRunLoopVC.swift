...
/**
    1、每一个线程维护一个runloop，如果不去获取，则没有runloop：
        runloop是一个对象，是事件循环机制，里面的主要源码是do-while循环。
        因为runloop一直do-while，所以线程就没有被销毁，但是如果线程没有runloop，那么该线程执行完它的任务就被销毁了。
        runloop是apple设计的一种数据结构而已，也可以理解为一种机制，一种模式，随便你怎么叫。
    ...
    3、mode中的source0与source1:
        官网说，其实不用太纠结，就是source1是内核调度(基于port)，source0是用户调度而已。
        source0是处理触摸事件，source1主要用于线程间通信。
        但用户使用起来，不用太在意这两者的区别。直接用添加source事件就可以了。
        例如点击按钮这些事件，都是通过source传递到runloop的。
        RunLoop 需要处理的消息，包括 time 、observer对象、 source 消息，它们都属于 Mode item。
        所有的 mode item 都可以被添加到 Mode 中，Mode 中包含可以包含多个 mode item，一个 item 可以被同时加入多个 mode。
        但一个 item 被重复加入同一个 mode 时是不会有效果的。如果一个 mode 中一个 item 都没有，则 RunLoop 会直接退出，不进入循环。

    
    4、CFRunLoopRef是用C语言写的Runloop，NSRunLoop是用OC语言写的runloop，而NSRunLoop是基于CFRunLoopRef的。
       NSRunLoop是foundation框架，CFRunLoopRef是Core Foundation框架里的(CF)。
 
    5、runloop的定时器不够精准，但是GCD的定时器是绝对精准的。
 
    6、runloop的自动释放池，池内其实就是timer，obsever，source这些对象的引用。
       在第一次启动runloop时，runloop就会创建一个释放池，当runloop最后一次销毁的时候，就会释放对象引用销毁，释放池。
        休眠和唤醒的时候也会销毁和创建 释放池。
