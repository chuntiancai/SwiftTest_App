...
/**
    ...
    3、LLDB指令：
        ...
        3.1、常用指令：
             ...
             读取内存的值命令：
                    memory read 内存地址        //可以省略memory read，直接x命令。
                    x/数量-格式-字节大小 内存地址                //读取内存中的值
                       x/3xw 0x00000000fe150e
                       x/4xg      --b,h,w,g对应1,2,4,8个字节；x代表十六进制，f是浮点，d是10进制，b：byte 1字节，h：half word 2字节，
    ...
    1、LLVM项目是模块化、可重用的编译器，同时也包含了工具链，是一个大集合。可以百度一下LLVM的官网。LLVM是一种架构、一个项目。
        ...
        编译器：
            ...
            LLVM：其实就是新增了跨平台的中间代码的优化器，跨平台是指跨编译器平台(GCC、Clang)， 也就是通过通过GCC、GHC等编译生成的前端代码，都可以通过LLVM架构生成LLVM架构的中间代码。
                  LLVM的中间代码有个专有名词：LLVM IR(.ll文件)。然后LLVM通过LLVM IR中间代码编译成不同机器平台的后端机器代码(例如x86、arm)。
...
/**
    1、编译相关命令
        clang -rewrite-objc main.m 命令，可以吧main.m文件编译成c++文件，也就是oc文件转换为c++文件。
 */
